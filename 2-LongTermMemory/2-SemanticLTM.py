...
def _semantic_search(query: str, user_id: str, limit: int = 5) -> list[dict]:
    """Search all category namespaces semantically and return top results."""
    results = []
...
def _load_all_memories(user_id: str) -> dict[str, list[dict]]:
    """Load all stored memories (used for 'memory' debug command)."""
    all_memories = {}
    for category in ["personal", "preferences", "goals", "relationships", "other"]:
        namespace = (user_id, category)
        try:
...
def inject_memories(state: AgentState, runtime: Runtime[Context]) -> dict[str, Any] | None:
    ...
    relevant_memories = _semantic_search(query=last_human.content, user_id=user_id, limit=5)

    if not relevant_memories:
        return None

    lines = ["Relevant facts about this user (retrieved by semantic similarity):"]
    for mem in relevant_memories:
        score_str = f" (score: {mem['score']:.2f})" if mem["score"] is not None else ""
        lines.append(f"  - [{mem['category']}] {mem['key']}: {mem['value']}{score_str}")
